...
// import useTanksData from '../../hooks/useTanksFromLocal'
...
function Blogs () {
  return fakeList.map(l => <Blog key={l.id} item={l}/>)
}
  ...
  return (
    <Link className="blog-overview" to={`/blog/${item.id}`}>
      ...
      <div className="blog-list-info">
        <div className="blog-list-title">{`${item.id}-${item.title}`}</div>
...
function RenderSlug ({slugs}) {
  return slugs.map((slug, index) => <span key={index}>#{slug}</span>)
}